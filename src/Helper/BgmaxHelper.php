...
namespace Lomart\Module\Bgmax\Site\Helper;
 ...
 **/
function bg_hex2hex(String $color)
{
    $color = trim($color, '#');
    if (strlen($color) == 6) {
        return $color;
 ...
 **/
function bg_hex2rgb(String $color)
{
    $color = trim($color, '#');
    if (strlen($color) == 6) {
 ...
 **/
function colorImageBottom(String $imageName)
{
    ...
    switch ($imgTyp) {
        case 1 : $im = imagecreatefromgif($imageName);
            break;
        case 2 :
        case ' ' : $im = imagecreatefromjpeg($imageName);
            break;
        case 3 : $im = imagecreatefrompng($imageName);
            break;
        default: {
            $app = Factory::getApplication();
            $app->enqueueMessage(TEXT::_('IMGNAME_ERROR').'[name='.$imageName.'] [ type='.$imgTyp.'] [ format= '.$imgW.'x'.$imgH, 'error');
            var_dump(gd_info());
            return "";
        }
    }
 ...
 *
 **/
function bg_formatDate(String $date)
{
    $tmp = explode('-', $date);
 ...
 **/
function getRandomImage(String $img_folder)
{
...
class BgMaxHelper
{
    ...
    public static function getBgMaxInfos(&$params, $modTitle)
    {
        ...
        ****/
        ...
        $now = Factory::getDate('now', $config->get('offset'));
        ...
        if (($bgFilter = $params->get('filterContent')) || ($bgmaxDebug)) {
            ...
            $bg_id = $app->getInput()->get('id', 0, 'int');
            $bg_menuid = $app->getInput()->get('Itemid', 0, 'int');
            $bg_option = trim($app->getInput()->get('option', 0));
            $bg_layout = trim($app->getInput()->get('layout', 0));
            $bg_view = trim($app->getInput()->get('view', 0));
            $bg_artid = '';
            $bg_catid = '';
            switch ($bg_view) {
                case 'article':
                ...
                case 'categories':
                    $bg_catid = intval($app->getInput()->get('id', 0));
                    break;
            }
            $context = $bg_option;
        ...
        if ($bgmaxDebug) {
            $msg = Text::_('INFO_DEBUG').'#OK# BGMAX - '.$modTitle.': <br>'.$period_msg;
            if (isset($context)) {
                $msg .= 'Context:'.$context;
            }
            $app->enqueueMessage($msg);
        ...
        ****/
        ...
        if ($bgImage) {
            ...
            if ($bgImage) {
                ...
                $bgImage = trim(Uri::base(), '/').$rep.$bgImage;
            }
        }
        ...
        if ((strstr($bgMode, 'repeat')) || ($bgMode == 'cover') || ($bgImage == "")) {
            ...
            switch ($bgVAlign) {
                case "center": $str .= 'vertAlign:"middle",';
                    break;
                case "bottom": $str .= 'vertAlign:"bottom",';
                    break;
            }
            if ($bgZIndex != '-1') {
                $str .= 'zIndex:'.$bgZIndex.',';
        ...
        if ($contentSelector) {
            ...
            if ($contentWidth) {
                ...
                switch ($contentAlign) {
                    case 'left': $str .= 'margin-left: 0;';
                        break;
                    case 'center': $str .= 'margin: 0 auto;';
                        break;
                    case 'right': $str .= 'margin-right: 0; margin-left: auto;';
                        break;
                }
            }
            if ($contentColor) {
